...
def generator(z):
    ...
    with tf.name_scope("generator_data"):
        with tf.variable_scope("generator"):
            ...
            img2=tf.reshape(img,[2,28,28,1])
    tf.summary.image("generator_data", img1)
    tf.summary.image("generator_data", img2)    
    return img
...
def run_gan(sess, G_train_step, G_loss, D_train_step, D_loss, G_extra_step, D_extra_step, batch_size=64, dim=96,\
            show_every=100, print_every=20, num_epochs=50):
    ...
    with tf.name_scope("running_GAN"):
        for it in range(max_iter):
            ...
            #tf.summary.image("Get_batch",minibatch[0:3])
            if it % show_every == 99:
...
with get_session() as sess:
    ...
    train_writer = tf.summary.FileWriter( "./mnist_summ5" , sess.graph)
