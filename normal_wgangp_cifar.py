...
def discriminator(x):
    with tf.variable_scope("discriminator"):
        ...
        conv3 = tf.layers.conv2d(inputs=act2,filters=256, kernel_size=[5, 5], strides=[2,2], padding="SAME")
...
def generator(z):
    with tf.name_scope("gen_block"):
        with tf.variable_scope("generator"):
            ...
            deconv4 = tf.layers.conv2d_transpose(act4, filters=3, kernel_size=5, strides=2, padding="same")
...
with get_session() as sess:
    ...
    train_writer = tf.summary.FileWriter( "./normal_wgangp_cifar", sess.graph)
