...
def wgan_gp_loss(logits_real, logits_fake, batch_size, x, G_sample):
    ...
    eps = tf.random_uniform(shape=[batch_size, 1, 1, 1], minval=-1, maxval=1)
    ...
    with tf.variable_scope('',reuse=True) as scope:
        ...
        # print(grad_D_x_hat)
        grad_norm = tf.sqrt(tf.reduce_sum(tf.square(grad_D_x_hat), reduction_indices=[3]))
...
def generator(z):
    with tf.name_scope("gen_block"):
        with tf.variable_scope("generator"):
            ...
            deconv1 = tf.layers.conv2d_transpose(act2, filters=128, kernel_size=4, strides=2, padding="same",kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))
            ...
            deconv2 = tf.layers.conv2d_transpose(act3, filters=64, kernel_size=4, strides=2, padding="same",kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))
            ...
            deconv4 = tf.layers.conv2d_transpose(act5, filters=3, kernel_size=4, strides=2, padding="same")
...
with get_session() as sess:
    ...
    train_writer = tf.summary.FileWriter( "./new_wgangp1", sess.graph)
