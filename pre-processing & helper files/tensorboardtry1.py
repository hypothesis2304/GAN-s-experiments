...
def discriminator(x):
	with tf.name_scope("discriminator"):
		with tf.variable_scope("discriminator"):		 
			...
			x3=tf.layers.dense(inputs=x2,units=128,activation=leaky_relu,use_bias=True)
...
def generator(z):
	with tf.name_scope("generator_data"):
		with tf.variable_scope("generator"):
			...
			x3=tf.layers.dense(inputs=x2,units=4096,activation=tf.nn.relu,use_bias=True)
...
def get_solvers(learning_rate=1e-4, beta1=0.5, beta2=0.999):
	with tf.name_scope("Train"):
		D_solver = tf.train.AdamOptimizer(learning_rate=1e-5, beta1=beta1, beta2=beta2)
		G_solver = tf.train.AdamOptimizer(learning_rate=5e-5, beta1=beta1, beta2=beta2)
...
def run_gan(sess, G_train_step, G_loss, D_train_step, D_loss, G_extra_step, D_extra_step, batch_size=128, dim=512,\
...
with get_session() as sess:
	...
	train_writer = tf.summary.FileWriter( "./tensorflow/cifarsummary1" , sess.graph)
